#!/Users/tuanhpham/anaconda3/bin/python
...
class SecondPaper:
    def __init__(self, a=100, b=1, c=20, n_min=5, n_max=100, lamb_max=1, lamb_min=0.05, number_of_lamb=20):
        ...
        self.lamb = np.asarray(np.linspace(lamb_min, lamb_max, number_of_lamb))[
    ...
    def market_volume(self):
        ...
        market_volume = self.second_coef * \
            (1 - 1/(self.nn - n_C + 1) + n_C /
             (np.multiply((self.nn-n_C+1), (n_C + 1 + np.multiply(self.ll, (n_C-1))))))
        return market_volume

    def market_price(self):
        n_C = self.final_stable_nC()
        P = self.first_coef*(1 + np.multiply(self.ll, (n_C-1))) / \
            (np.multiply((self.nn-n_C+1), (n_C + 1 + np.multiply(self.ll, (n_C-1))))) + self.c
        return P
        ...
        for i in range(self.d1):
            for j in range(self.d2):
                if np.isnan(stable_nC[i, j]) and not np.isnan(n_C2[i, j]):
                    ...
                    # fill the data with a complete cartel for the other cases
                    stable_nC[i, j] = j + self.n_min
        return stable_nC
    ...
    def bar3D_plot(self):
        ...
        ax.bar3d(self.ll.flatten(), self.nn.flatten(), np.zeros(self.final_stable_nC().size),
                 0.02*np.ones(self.final_stable_nC().size), 0.02 *
                 np.ones(self.final_stable_nC().size),
        ...
        def update(ii):
            ...
            for k in reversed(index_list):
                if k >= i:
                    ...
                    ax.text(lambd[k] + 0.01, Q[k] - 0.5,
                            s='$\lambda$ = {:.2f}'.format(lambd[k]), fontsize=20)
    ...
    def plot_2D(self):  # best for case of n=50
        plt.figure()
        plt.scatter(self.ll, self.nn, s=self.n_C()/(int(self.n_max/50)),
                    cmap='viridis', alpha=0.5, c='k')
        ...
        for member in self.legend_array():
            plt.scatter([], [], c='k', alpha=0.5, s=member, label=str(
                member) + " members")  # add another empty scatter
